...
%         audiometry_dB= [ 0;    0;    0;    0;    0;   0];   %Pure tone threshold in dB SPL
        ...
        %------------------------------------------------------------------
        channelBFs = [250 500 1000*2.^(0:0.5:3)];
        bwOct = [1  1  1  0.5  0.5  0.5  0.5  0.5  0.5 ]; 
...
%             TM_dBO = obj.audiometry_dB - obj.mainGain_dB + obj.TM_dBSPL;
        ...
        function value = get.bwOct_INTERP(obj)
            value = obj.bwOct;%repmat(obj.bwOct, 1, obj.numChannels);
        end
...
%             assert(all(value==1/1) || all(value==1/2) || all(value==1/3) || all(value==1/4) || all(value==1/5), 'must be one of 1./(1:5)')
            obj = obj.flushAidData;
            obj.bwOct = value;
        end
    ...
    methods(Static)        
        ...
        %**********************************************************        
        function fullArray = interpPars(shortArray, DUMMY)
            numBands = numel(shortArray) + 5;
            nGUIbands = numel(shortArray);
            if numBands == nGUIbands
                fullArrayTMP = shortArray;
            else
                numSpacers = (numBands-nGUIbands) / (nGUIbands-1);
                fullArrayTMP = shortArray(1);
                for nn = 2:nGUIbands
                    fullArrayTMP = [fullArrayTMP,...
