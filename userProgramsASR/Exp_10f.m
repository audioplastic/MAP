...
for nn = 4*recConditions+1:5*recConditions    
    ...
    xR{nn}.numWavs = testWavs; %MAX = 358
    xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
    xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = 0.008;', 'DRNLParams.MOCrateThresholdProb =57;','DRNLParams.MOCtau =0.17;'};
...
for nn = 5*recConditions+1:6*recConditions   
    ...
    xR{nn}.numWavs = testWavs; %MAX = 358
    xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
    xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = 0.010;', 'DRNLParams.MOCrateThresholdProb =57;','DRNLParams.MOCtau =0.17;'};
...
for nn = 6*recConditions+1:7*recConditions   
    ...
    xR{nn}.numWavs = testWavs; %MAX = 358
    xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
    xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = 0.012;', 'DRNLParams.MOCrateThresholdProb =57;','DRNLParams.MOCtau =0.17;'};
...
for nn = 7*recConditions+1:8*recConditions   
    tmpIdx=tmpIdx+1;
    xR{nn} = xL; %simply copy the "Learn" object and change it a bit below
    recFolder = fullfile(expFolder,['FIX10featR_nz' num2str(nzLevel(tmpIdx))]);
    xR{nn}.opFolder = recFolder;    
    
    %These are the interesting differences between training and testing
    xR{nn}.numWavs = testWavs; %MAX = 358
    xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
    xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = -0.3162;', 'DRNLParams.MOCrateThresholdProb =57;','DRNLParams.MOCtau =0.17;'};
