...
for nn = 1:recConditions    
    ...
    recFolder = fullfile(expFolder,['NEWNONEfeatR_nz' num2str(nzLevel(nn))]);
...
for nn = 1*recConditions+1:2*recConditions    
    tmpIdx=tmpIdx+1;
    xR{nn} = xL; %simply copy the "Learn" object and change it a bit below
    recFolder = fullfile(expFolder,['NEWfeatR_cond' num2str(nn)]);
    xR{nn}.opFolder = recFolder;    
...
%     tmpIdx=tmpIdx+1;
%     xR{nn} = xL; %simply copy the "Learn" object and change it a bit below
%     recFolder = fullfile(expFolder,['FIXfeatR_cond' num2str(nn)]);
%     xR{nn}.opFolder = recFolder;    
%     
%     %These are the interesting differences between training and testing
%     xR{nn}.numWavs = testWavs; %MAX = 358
%     xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
%     xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = -0.02;'};
%     
%     %Now just to wrap it up ready for processing
%     if isMasterNode && ~isdir(xR{nn}.opFolder)
%         mkdir(xR{nn}.opFolder);
...
%     xR{nn}.numWavs = testWavs; %MAX = 358
%     xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
%     xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = -0.40;'};    
%     
%     %Now just to wrap it up ready for processing
%     if isMasterNode && ~isdir(xR{nn}.opFolder)
%         mkdir(xR{nn}.opFolder);
...
%     xR{nn}.numWavs = testWavs; %MAX = 358
%     xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
%     xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = -0.80;'};    
%     
%     %Now just to wrap it up ready for processing
%     if isMasterNode && ~isdir(xR{nn}.opFolder)
%         mkdir(xR{nn}.opFolder);
