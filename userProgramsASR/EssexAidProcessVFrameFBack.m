...
% DRNL
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MOCcontrol = zeros(21,6912); %CHANGE THIS LINE
...
%     rmsIPR(filterCount) = 20*log10(  sqrt(mean(yR.^2)) /2e-5 ); %rms in in dB SPL
    ...
    rmsIPL(filterCount) = 20*log10(  abs(yL(end)) /2e-5 ); %rms in in dB SPL
...
%     disp(MOCthreshold_dBOP)
    ...
    MOCnow = max(MOCnow, 0) * MOCfactor + 0.000000001; %the tiny offset is due to a crummy eml bug GRR!
    MOCnow = 10.^(-MOCnow/20);
