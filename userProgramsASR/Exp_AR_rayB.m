function Exp_AR_rayB(isMasterNode)
...
xL.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb=0;', 'OMEParams.rateToAttenuationFactorProb=0.012;' };
...
for zz = 0
    tmpIdx=0;
    for nn = zz*recConditions+1:(zz+1)*recConditions
        tmpIdx=tmpIdx+1;
        xR{nn} = xL; %simply copy the "Learn" object and change it a bit below
        recFolder = fullfile(expFolder,['w012AR'    num2str(nn)]);
        xR{nn}.opFolder = recFolder;
        
        %These are the interesting differences between training and testing
        xR{nn}.numWavs = testWavs; %MAX = 358
        xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
%         xR{nn}.MAPparamChanges= {['DRNLParams.rateToAttenuationFactorProb=-10^(-' num2str(zz) '/20);']};
