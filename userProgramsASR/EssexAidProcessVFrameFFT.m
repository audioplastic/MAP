%#eml
function [ frameBufferL, frameBufferR, filterStatesL, filterStatesR,  ARampL, ARampR, MOCend, peakIPL, peakOPL, rmsIPL, rmsOPL, peakIPR, peakOPR, rmsIPR, rmsOPR, MOCcontrol, FILO_iL, FILO_iR, FILO_oL, FILO_oR ] =...
...
yL=stapesVelL;
yR=stapesVelR;
% xmzpL = zeros(MAX_SAMP,1);
% xmzpR = zeros(MAX_SAMP,1);
xmzpL = [yL; zeros(nfft-numSamples,1)]; % zero pad the signal
xmzpR = [yR; zeros(nfft-numSamples,1)]; % zero pad the signal
XmL = fft(xmzpL,nfft);
XmR = fft(xmzpR,nfft);

for filterCount=1:numChannels
    
    MOCnow = MOCcontrol( 1:numSamples, filterCount ); %syntactic shorthand
...
%     size()
    ...
    YmL = XmL .* H(1:nfft,filterCount);   % freq domain multiplication
    YmR = XmR .* H(1:nfft,filterCount);   % freq domain multiplication
    ymL = real(  ifft(YmL)  );         % inverse transform % REAL there for eml coder
    ymR = real(  ifft(YmR)  );         % inverse transform % REAL there for eml coder
    FILO_iL(1:nfft,filterCount) =   [FILO_iL(numSamples+1:end,filterCount); zeros(numSamples,1)] + ymL   ; %first in, last out buffer
    FILO_iR(1:nfft,filterCount) =   [FILO_iR(numSamples+1:end,filterCount); zeros(numSamples,1)] + ymR   ; %first in, last out buffer
    yL = FILO_iL(1:numSamples,filterCount); % overlap add
    yR = FILO_iR(1:numSamples,filterCount); % overlap add
...
%     end
    ...
    xmzpLo = [yL; zeros(nfft-numSamples,1)]; % zero pad the signal
    xmzpRo = [yR; zeros(nfft-numSamples,1)]; % zero pad the signal
    XmLo = fft(xmzpLo,nfft);
    XmRo = fft(xmzpRo,nfft);
    YmL = XmLo .* H(1:nfft,filterCount);   % freq domain multiplication
    YmR = XmRo .* H(1:nfft,filterCount);   % freq domain multiplication
    ymL = real(  ifft(YmL)  );         % inverse transform % REAL there for eml coder
    ymR = real(  ifft(YmR)  );         % inverse transform % REAL there for eml code
    FILO_oL(:,filterCount) = [FILO_oL(numSamples+1:end,filterCount); zeros(numSamples,1)] + ymL   ; %first in, last out buffer
    FILO_oR(:,filterCount) = [FILO_oR(numSamples+1:end,filterCount); zeros(numSamples,1)] + ymR   ; %first in, last out buffer
    yL = FILO_oL(1:numSamples,filterCount); % overlap add
    yR = FILO_oR(1:numSamples,filterCount); % overlap add
