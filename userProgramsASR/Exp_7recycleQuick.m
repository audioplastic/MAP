...
xL.noisePreDur = 2;
xL.noisePostDur = 0.1;
xL.truncateDur  = 1.9; %Dr. RF used 0.550
...
for nn = 0*recConditions+1:1*recConditions    
    ...
    recFolder = fullfile(expFolder,['TC1000featR_cond' num2str(nn)]);
    xR{nn}.opFolder = recFolder;    
    
    %These are the interesting differences between training and testing
    xR{nn}.numWavs = testWavs; %MAX = 358
    xR{nn}.noiseLevToUse = nzLevel(tmpIdx);
    xR{nn}.MAPparamChanges= {'DRNLParams.rateToAttenuationFactorProb = 7;','DRNLParams.MOCrateThresholdProb = 85;', 'DRNLParams.MOCtau = 1;'};
