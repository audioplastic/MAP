...
OMEParams.rateToAttenuationFactorProb=0.03; 
...
OMEParams.ARdelay=efferentDelay;  %Moss gives 8.5 ms AR latency
...
DRNLParams.ctBMdB = 20; %Compression threshold dB re 10e-9 m displacement
...
DRNLParams.p=0.4;   DRNLParams.q=100;   % save p and q for printing only
...
DRNLParams.rateToAttenuationFactor = .02;  % strength of MOC
...
% IHC_cilia_RPParams.Et=	0.110;          % endocochlear potential (V)

IHC_cilia_RPParams.Gk=	2e-008;         % 1e-8 potassium conductance (S)
IHC_cilia_RPParams.Gk=	2.3e-008;         % 1e-8 potassium conductance (S)
...
LSRtauCa=40e-6;            HSRtauCa=110e-6;  % seconds
...
AN_IHCsynapseParams.r=	400; % *reuptake rate from cleft into cell
...
MacGregorParams.tauGk=	0.0007;     % K+ conductance tau (s)
...
if nargin>3 && ~isempty(paramChanges)
    ...
    for idx=1:nChanges
        ...
        if ~isempty(x)
            if ~strcmp(x(end),';')
                disp(['paramChanges error (terminate with semicolon) ' x])
                error
            end
